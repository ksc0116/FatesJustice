...
#pragma endregion
	...
	PhysicsSystem::SimplexResult PhysicsSystem::DoSimplex2(Simplex& simplex, ZeldaMath::Vector3D& direction)
	{
		...
		// 아래의 코드는 필요가 없을거 같은데? 이미 DoSimplex 밖에서 해준다... (2023.08.01 주석처리, 별 문제 없다면 이후 완전히 삭제)
		//float dotBA_BO = DotProduct(BANormal, BONormal);

		//// 선분 BA와 수직인 직선이 만드는 공간 안에 원점이 존재하는 경우
		//if (dotBA_BO >= 0.0f)
		//{
		//	direction = CrossProduct(CrossProduct(BANormal, BONormal), BANormal);

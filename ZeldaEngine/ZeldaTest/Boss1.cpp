...
void Boss1::Start()
{
	...
	//bossParticleObject->GetTransform()->SetLocalPosition(Vector3D(0, -5, 0));
...
void Boss1::Update()
{
	...
	//if (Input::GetInstance().PressedKeyNow(VK_F6) && EngineSetting::GetInstance().GetShowDebugInfo())
	//{
	//	RevertToPhaseTwo();
	//}
			...
			boss1Dotween->DONothing(0.7f + bossRageLookDuration).OnComplete([=]()
				{
					...
					cameraDotween->DOShakePosition(0.3f, 1.0f).SetDelay(0.2f).OnComplete([=]()
						{
							rageTextureObject->SetActive(false);
							boss1Dotween->DONothing(0.4f).SetDelay(1.0f).OnComplete([this]()
								{
									ChoosePattern();
...
void Boss1::GroggyEngage()
{
	...
	boss1Dotween->DONothing(groggyTime - 2.0f).SetEase(EaseBasic).OnComplete([this]()
		{
			// 시간이 지난 후 이미 페이즈가 넘어간 상태라면 재생하지 않는다.
			if (patternNum == int(Boss1State::Groggy) || phaseNumber != 2)
			{
				m_animator->Play(L"root|011-2.Groogy->Idle");
			}
		});

	boss1Dotween->DONothing(groggyTime).OnComplete([this]()
		{
			isGroggyEnd = true;
		});
}
...
void Boss1::Pattern3Function(double speed, double walkSpeed, int attackCount)
{
	...
	Vector3D playerPosition = Vector3D(m_playerTransform->GetLocalPosition().x, 4.0f, m_playerTransform->GetLocalPosition().z);
	...
	Vector3D destinationVector = m_gameObject->GetTransform()->GetLocalPosition() + (playerPosition - m_gameObject->GetTransform()->GetLocalPosition()) * 0.7f;
	...
	boss1Dotween->DOMove(destinationVector, distance / walkSpeed).OnComplete([=]()
		{
			if (distance != 0)
...
void Boss1::Pattern5Function(double speed)
{
	...
	boss1Dotween->DONothing(0.6f / speed).OnComplete([=]()
		{
...
void Boss1::Pattern6Function(double speed, double walkSpeed)
{
	...
	Vector3D playerPosition1 = Vector3D(m_playerTransform->GetLocalPosition().x, 4.0f, m_playerTransform->GetLocalPosition().z);
	...
	Vector3D destinationVector = m_gameObject->GetTransform()->GetLocalPosition() + (playerPosition1 - m_gameObject->GetTransform()->GetLocalPosition()) * 0.7f;
...
void Boss1::PatternMoveFunction(Vector3D p_position, bool isPattern, double duration /*= 1.0f*/, double speed)
{
	...
	if (!isPattern)
	{
		...
		boss1Dotween->DOMove(m_gameObject->GetTransform()->GetLocalPosition() - Vector3D(0, 10, 0), 3.0f).SetDelay(0.5f).SetEase(EaseInCubic).OnComplete([=]()
			{
	...
	else if (isPattern)
	{
		...
		boss1Dotween->DOMove(p_position, distance / speed).OnComplete([=]()
			{
				m_animator->Play(L"root|001-3.Walk->Idle");
...
void Boss1::ChoosePattern()
{
	...
	else if (phaseNumber == 2)
	{
		...
		else
		{
			...
			// 족자 2개일 때 강화 나뭇가지 패턴
			else if (currenthp <= hpPerScroll * 2)
				patternNum = int(Boss1State::Pattern1);
...
void Boss1::ApplyDamage(int phasenum, bool isRageOn)
{
	...
	// 2페이즈 광폭화 후
	else if (phasenum == 2 && isRageOn == false)
	{
		branchDamage = 17;
		chargeDamage = 12;
		normalAttackDamage = 9;

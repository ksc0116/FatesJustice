...
void GameManager::Start()
{
	...
	else if (SceneManager::GetInstance().GetName() == L"Directing Scene 1")
	{
	...
	{
		/// 게임 오버 관련 사운드 필요
		//SoundManager::GetInstance().PlaySFX(SoundManager::Effect::GameOver);
	}
	if (SceneManager::GetInstance().GetName() == L"MainScene")
	{
		SoundManager::GetInstance().PlayBGM(SoundManager::BGM::Opening);
...
void GameManager::ShowPhase2EndEvent()
{
	...
	PushSceneEvent(4.0f, L"Directing Scene 1");
}
...
{
	PushAnimationEvent(2.0f, mainAnimator, 0.0f, 1.5f, 1.0f, 0.5f, 0.0f,
		{
			L"Black"
		}
	);

	PushSceneEvent(4.5f, L"GameOverScene");
}
